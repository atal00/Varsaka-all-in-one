...
export function ArticleImageClient({ articleId, imageUrls, alt }: { articleId: string, imageUrls: string[], alt: string }) {
  ...
  const handleRefresh = async () => {
    ...
    try {
      const res = await fetch(`http://127.0.0.1:3001/articles/${articleId}/refresh-image`, {
        method: 'POST'
      });
      if (res.ok) {
        router.refresh();
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-1 p-1">
        {imageUrls.map((url, index) => (
          <div key={index} className="relative h-64 md:h-80 group overflow-hidden bg-zinc-200 dark:bg-zinc-900 rounded-lg">
            {/* Standard img tag is more reliable for slow AI generation endpoints than Next.js Image */}
            <img 
              src={url} 
